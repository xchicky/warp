...
const LOCAL_DIRECT_PLAN_MODE_PROMPT: &str = "You are in plan mode. Inspect the workspace using only read-only tools, search_codebase, web_search, web_fetch, suggest_shell_command, and read-only subagents. Do not modify files, execute commands, call MCP tools, or update todos. When planning file creation or modification, keep every path under the current working directory subtree; files outside the cwd subtree cannot be written, such as /tmp/... when cwd is /Users/.... Your plan must be specific and actionable: name the exact intended tool for each implementation step, such as write_file path=src/lib.rs or run_shell_command 'cargo test'. Include exact paths and commands so the user can review and catch path or tool mistakes before approving. CRITICAL: Plan mode does NOT show your plan to the user via normal assistant messages. You MUST call the exit_plan_mode tool with your complete plan to present it for review. Inline plan text in normal assistant output will be invisible to the user. The user will only see your plan after you call exit_plan_mode. When ready, call exit_plan_mode with the complete plan. No changes will be executed in plan mode.";
...
fn exit_plan_mode_tool_definition() -> OpenAIChatTool {
    OpenAIChatTool {
        ...
        function: OpenAIChatToolFunction {
            ...
            description: "Finish plan mode by presenting the complete plan to the user for review. This is the ONLY way to surface your plan to the user; inline plan text in normal assistant output will not be shown as a reviewable plan. This does not execute any changes.".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "plan": {
...
mod tests {
    ...
    fn local_plan_mode_system_prompt_is_injected() {
        ...
        *user_query_mode = UserQueryMode::Plan;
        ...
        assert!(system_prompt.contains("under the current working directory subtree"));
        assert!(system_prompt.contains("files outside the cwd subtree cannot be written"));
        assert!(system_prompt.contains("Your plan must be specific and actionable"));
        assert!(system_prompt.contains("write_file path=src/lib.rs"));
        assert!(system_prompt.contains("run_shell_command 'cargo test'"));
        assert!(system_prompt.contains("Include exact paths and commands"));
